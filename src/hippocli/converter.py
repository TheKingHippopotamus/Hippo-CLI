...
def read_json_with_stock_price(
    company_json: Path, stock_price_json: Path | None = None
        ...
        if isinstance(nested_data, dict):
            ...
            try:
                if not isinstance(nested_data, (list, tuple)):
                    ...
                    nested_data = (
                        dict(nested_data)
                        if hasattr(nested_data, "__iter__") and not isinstance(nested_data, str)
                        else {}
                    )
                    if isinstance(nested_data, dict):
                        for key, value in nested_data.items():
...
def _split_dataframes(
    df: pl.DataFrame, stock_price_df: pl.DataFrame | None = None
) -> Tuple[pl.DataFrame, pl.DataFrame]:
    """
    ...
    stock_price_df = (
        pl.DataFrame(stock_price_records)
        if stock_price_records
        else pl.DataFrame(
            schema={
                "company_id": pl.Int64,
                "ticker": pl.String,
...
def _stringify_nested(df: pl.DataFrame, exclude: Iterable[str] = ()) -> pl.DataFrame:
    ...
    for col in df.columns:
        ...
        if isinstance(df[col].dtype, nested_types):
            df = df.with_columns(pl.col(col).map_elements(_encode, return_dtype=pl.String))
    return df
...
def json_to_parquet(
    ...
    if stock_price_parquet:
        ...
        logger.info(
            "Wrote stock price Parquet to %s (%d records)", stock_price_parquet, stock_price_count
        )
...
def json_to_sql(
    ...
    if stock_price_sql and stock_price_df.height > 0:
        ...
        with stock_price_sql.open("w", encoding="utf-8") as f:
            ...
            f.write(
                f"INSERT INTO {stock_price_table_name} (company_id, ticker, ts, value, interval, valueUnit) VALUES\n"
            )
            values_lines = []
